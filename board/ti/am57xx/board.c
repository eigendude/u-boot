...
int spl_start_uboot(void)
{
	...
	if (board_is_bbai()) {
		...
		for ( addr = CAPE_EEPROM_ADDR0; addr <= CAPE_EEPROM_ADDR3; addr++ ) {
			...
			rc = i2c_get_chip_for_busnum(1, addr, 1, &dev);
			if (rc) {
				pr_err("Checking (ret 1 bus works) I2C2 bus. ret %d\n", rc);
			}
			rc = i2c_get_chip_for_busnum(2, addr, 1, &dev);
			if (rc) {
				pr_err("Checking (ret 1 bus works) I2C3 bus. ret %d\n", rc);
			}
